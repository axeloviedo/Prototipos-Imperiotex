...
| K4 | ~~**Grupos de proveedores = los del Excel**~~ *(revisada por S1, 2026-09-18: ahora son los **tipos** de proveedor)* | TEL Telas · AVI Avíos · SRV Servicios · GEN Generales (el proveedor guarda el código). Los proveedores de servicios de producción que faltaban (PROV-0005..0021) se crearon en el grupo SRV con RUC/DNI **a confirmar**. |
...
| L1 · nota | **Precio mínimo en Comercial: siempre** *(2026-09-18, decidido por el usuario)* | En Comercial el precio mínimo del artículo se exige **siempre**, sin depender de «Verificar el precio mínimo»: ni listas de precios, ni ofertas, ni precio a mano, ni descuento manual lo pueden bajar; con mínimo 0, el precio debe ser mayor que cero. El precio de las listas es referencial. Detalle: `COMERCIAL/docs/12-prototipo-diseno.md` §12 (LP8, LP11, LP12). |
...
## Q · Producción: referencia, empresa, lista, fecha y stock sin negativos (2026-09-17, rama `feat/produccion-referencia-stock-sin-negativos`)

| # | Decisión | Detalle |
|---|---|---|
| Q1 | **N° Referencia: «Nueva» o «Vincular a existente»** | La referencia sigue siendo un agrupador de órdenes (J5) que nace en Producción, nunca en la SF. Al crear órdenes (PR-01 Nueva OF, PR-03 desde una SF y el reproceso) el campo es un selector: **Nueva** asigna el correlativo al crear; **Vincular** ofrece las referencias con órdenes abiertas (origen, cantidad de órdenes y artículos) y suma las nuevas órdenes a esa referencia para el listado por fase y el recosteo. No se crean referencias «a mano»: un valor que no existe se rechaza (`Prod._ref`). |
| Q2 | **El Disponible nunca queda negativo** *(ajusta T7)* | Nadie compromete más de lo que hay: `Stock.comprometerHasta` reserva solo hasta el Disponible. La **SF aprobada** compromete la materia prima bruta hasta lo disponible y guarda por material `req` (requerido) y `cant` (comprometido); la diferencia se pide con la Solicitud de Materiales de GI-23. La **orden liberada** compromete lo suyo hasta lo disponible (ya era así). **Reserva al llegar** (`Docs.reserva`): la OC y la Solicitud de Transferencia que atienden una SOL heredan su enlace `of` / `sf`; al recibir la OC (GI-09) o confirmar la transferencia (GI-11), lo que llega se compromete en el acto para la **orden liberada** y, si no, para la **SF aprobada** que lo pidió, solo por lo que aún le falta. Si la orden ya no está abierta o la SF ya se convirtió, lo recibido queda disponible. El **Pedido** (T1) sigue siendo informativo: es «lo que viene en camino» y la orden lo muestra por línea. La transferencia sigue comprometiendo el origen al aprobarse (T2/T7), validando disponible. |
| Q3 | **Emisión y recibo solo con lo que la orden puede usar** | **Puede usar la orden** = Disponible del almacén + lo que la propia orden tiene comprometido ahí (`Prod.dispPara`); lo reservado por otros documentos no se toca. Si una línea no alcanza, la **emisión se bloquea** con el detalle (hay, comprometido por otros, puede usar); el botón **«Emitir lo disponible y pedir el resto»** emite lo que hay y pide la diferencia a Logística. El **recibo** valida igual las líneas Notificación. La pestaña Materiales muestra **Comprometido · Pedido (en camino) · Falta pedir** y el botón **«Pedir a Logística lo que falta»** (una SOL por almacén, que al llegar se compromete para la orden, Q2). |
| Q4 | **Almacenes por empresa en Producción** | Nueva OF, «Entra en» de PR-03, líneas de Materiales, PR-05a y Tercerizar ofrecen solo los almacenes **activos de la empresa**: la activa (selector de la barra, `BD.empresa`, ahora funcional en Producción) o la de la SF / la orden. La orden **no toma material de otra empresa**: se valida al crearla, al cambiar el almacén de una línea y al liberar. |
| Q5 | **Lista de materiales: almacén donde entra lo producido y observación, opcionales** *(matiza N10)* | La cabecera de la LDM (GI-17) gana **«Almacén donde entra lo producido»** (opcional, no puede ser de tránsito) y **«Observación»** (texto libre, p. ej. con quién se terceriza). Producción propone ese almacén al crear la orden (`Prod.almRecibo(art, ldm)`); si está vacío usa la heurística anterior (el almacén desde donde otras listas toman el artículo) y, si tampoco hay, obliga a elegir (L7). N10 sigue vigente en lo que decía: no hay tipos de lista y el almacén de **cada componente** se elige por línea. |
| Q6 | **Fecha requerida en vez de mes proyectado** | La SF ya no lleva «Mes proyectado»: trabaja solo con **Fecha requerida**, obligatoria al enviar a revisión (la fecha contiene al mes). La bandeja GI-21 filtra por **mes requerido** derivado de la fecha. La orden hereda la fecha de la SF (`of.fechaFin`), Nueva OF permite indicarla y la ficha la muestra. |
| Q7 | **La tercerización se queda como está** | No hay indicador explícito en la LDM: una lista está tercerizada cuando un componente se toma de un almacén **en tránsito** y lleva un recurso SERVICIO DE TERCEROS (O4). La unidad de tercerización es la **orden** (= artículo con lista): si un proceso interno debe salir a terceros, se separa en su propio artículo en proceso (como CRUDO → LAVADO en Zuleika). Para saberlo de un vistazo se usa la **Observación** de la lista (Q5). |

---

## S · Socios de negocio: grupo y tipo (2026-09-18)

> Pedido del usuario: «en proveedores el tipo en realidad es grupo y el grupo es tipo; los grupos son nacional e internacional y el tipo son los tipos que hemos definido; en clientes también agregas grupo (nacional, internacional)». Se cambian **pantalla y datos**.
