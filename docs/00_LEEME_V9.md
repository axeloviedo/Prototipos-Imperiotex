...
- Un solo documento para productos y servicios (el servicio es un artículo no inventariable). Listas de precios y ofertas: se crea la lista (moneda, sede y segmento opcionales; con fechas = oferta) y se le agregan artículos o grupos con precio o % (sede + segmento → sede → segmento → general → precio sugerido de GI-02; la oferta vigente manda), descuento en rango, precio mínimo y multimoneda con reversión.
