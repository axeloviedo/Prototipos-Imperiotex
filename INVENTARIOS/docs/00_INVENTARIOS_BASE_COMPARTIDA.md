...
| GI-25/26 Modelos | `maestros.modelos` · `articulos.modelo` | **U1–U7** (por ahora solo MOD-0001, el terminado ZULEIKA; sin atributo Acabado): listado (código, plantilla, artículos, preseleccionado, estado) y ficha: plantilla ordenada (↑↓, agregar, quitar), artículo preseleccionado, tabla de artículos con sus valores editables (marca combinaciones repetidas), «+ Agregar artículo existente» (solo sin modelo), «⚙ Generar combinaciones» (valores marcados × artículo base → artículos nuevos con código del grupo y nombre sugerido editable; sin códigos de barras ni LDM) y Desactivar/Activar. GI-02 suma el campo **Modelo**: con modelo, la pestaña Atributos muestra la plantilla fija. GI-01 suma columna y filtro de Modelo. |
