...
const PRMAT = {
  ...
  render(of) {
    ...
    const filas = [];
    of.mats.forEach((m, i) => {
      const pend = UI.r4(Math.max(0, m.plan - m.consumido)), disp = Stock.disp(m.alm, m.cod), ped = Prod.pedido(of, m.cod, m.alm);
      ...
      filas.push('<tr><td>' + (++n) + '</td><td>Artículo</td><td>' + m.cod + '</td>' +
        ...
        '<td class="num">' + UI.n(m.plan) + '</td><td class="num">' + UI.n(m.comp) + '</td><td class="num">' + UI.n(m.consumido) + '</td><td class="num">' + UI.n(pend) + '</td>' +
        '<td class="num"><span class="' + (disp + 0.0001 < pend - m.comp ? 'err-t' : '') + '">' + UI.n(disp) + '</span></td><td>' + quitar('Artículo', i) + '</td></tr>');
    });
    of.recs.forEach((r, i) => {
      const R = M.rec(r.cod) || {};
      filas.push('<tr><td>' + (++n) + '</td><td>Recurso</td><td>' + r.cod + '</td><td>' + UI.esc(R.nom || r.cod) + (Prod.esServicio(r.cod) ? '<br><span class="mini">servicio de terceros · costo estándar ' + UI.s(R.costo) + '</span>' : '') + '</td>' +
        ...
        '<td class="num">' + UI.n(r.plan) + '</td><td class="num">—</td><td class="num">' + UI.n(r.real) + '</td><td class="num">' + UI.n(Math.max(0, r.plan - r.real)) + '</td><td class="num">—</td><td>' + quitar('Recurso', i) + '</td></tr>');
    });
    of.textos.forEach((t, i) => {
      filas.push('<tr><td>' + (++n) + '</td><td>Texto</td><td class="mini">—</td>' +
        '<td colspan="10">' + (ed ? '<input value="' + UI.esc(t) + '" placeholder="Texto / instrucción" onchange="PRMAT.cambiar(\'Texto\',' + i + ',\'txt\',this.value)" style="width:100%;border:1px solid var(--borde);border-radius:5px;padding:5px 7px">' : UI.esc(t)) + '</td><td>' + quitar('Texto', i) + '</td></tr>');
    });
    return '<div class="card" style="padding:12px 16px"><div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><b style="font-size:13px">Detalle de la orden</b>' +
      ...
      (ed ? '<button class="btn btn-secondary btn-sm" onclick="PRMAT.agregarArt()">+ Artículo</button><button class="btn btn-secondary btn-sm" onclick="PRMAT.agregarRec()">+ Recurso</button><button class="btn btn-secondary btn-sm" onclick="PRMAT.agregarTxt()">+ Texto</button>' : '') + '</div></div>' +
      UI.tabla([['#', '', '36px'], ['Tipo', '', '80px'], ['Código', '', '100px'], 'Componente / Descripción', ['Cantidad base', 'num'], 'Unidad', 'Almacén', 'Método', ['Cantidad planificada', 'num'], ['Stock Comprometido', 'num'], ['Stock Consumido', 'num'], ['Pendiente', 'num'], ['Stock Disponible', 'num'], ['', '', '70px']],
