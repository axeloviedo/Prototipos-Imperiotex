/* PRODUCCION · PR-08 Existencias y movimientos de TODA la base compartida (todos los módulos, con filtros) · PR-09 Costos y eficiencia */
const PR08 = {
  tab: 'ex', f: { alm: '', grupo: '', q: '', cero: false, tipo: '', tipoMov: '', modulo: '', malm: '', mq: '', art: 'PT-0001', kalm: '' },
  render() {
    const t = PR08.tab;
    const tabs = [['ex', 'Existencias'], ['mov', 'Movimientos (' + BD.d.movs.length + ')'], ['kar', 'Kardex']];
  ...
  movimientos() {
    const f = PR08.f;
    const q = f.mq.toLowerCase();
    const lista = BD.d.movs.filter(m => (!f.tipo || m.tipo === f.tipo) && (!f.tipoMov || m.tipoMov === f.tipoMov) && (!f.modulo || (m.modulo || '') === f.modulo) &&
      (!f.malm || m.lineas.some(l => l.alm === f.malm)) && (!q || [m.ndoc, m.doc, m.id, m.det, m.obs].join(' ').toLowerCase().includes(q)));
    const modulos = [...new Set(BD.d.movs.map(m => m.modulo || ''))].filter(Boolean).sort();
    const tipos = (BD.d.maestros.tiposMovimiento || []).map(t => ({ v: t.cod, t: t.cod + ' · ' + t.nom }));
    return '<div class="card"><div class="filters">' +
      UI.campo('Movimiento', '<select onchange="PR08.f.tipo=this.value;App.refrescar()">' + UI.opts(['Ingreso', 'Salida', 'Transferencia'], f.tipo, 'Todos') + '</select>') +
      UI.campo('Tipo de movimiento', '<select onchange="PR08.f.tipoMov=this.value;App.refrescar()" style="max-width:260px">' + UI.opts(tipos, f.tipoMov, 'Todos') + '</select>') +
      UI.campo('Módulo', '<select onchange="PR08.f.modulo=this.value;App.refrescar()">' + UI.opts(modulos, f.modulo, 'Todos') + '</select>') +
      UI.campo('Almacén', '<select onchange="PR08.f.malm=this.value;App.refrescar()">' + UI.opts(M.ALMACENES.map(a => ({ v: a.cod, t: a.cod })), f.malm, 'Todos') + '</select>') +
      UI.campo('Buscar (documento / movimiento)', '<input value="' + UI.esc(f.mq) + '" onchange="PR08.f.mq=this.value;App.refrescar()" placeholder="Ej. OF-000001">') + '</div></div>' +
      UI.tabla(['Movimiento', 'Fecha', 'Tipo', 'Tipo de movimiento', 'Detalle', 'Documento', 'Módulo', 'Origen → destino', ['Líneas', 'num'], ['Valor', 'num']], lista.map(m => {
        const col = { Ingreso: 'var(--confirmado)', Salida: 'var(--parcial)', Transferencia: 'var(--aprobada)' }[m.tipo];
        return '<tr class="clickable" onclick="PR08.verMov(\'' + m.id + '\')"><td><b>' + m.id + '</b></td><td class="mini">' + m.fecha + '</td><td>' + UI.badge(m.tipo, col) + '</td>' +
          '<td class="mini"><b>' + UI.esc(m.tipoMov || '') + '</b><br>' + UI.esc(m.tipoMovNom || '') + '</td><td>' + UI.esc(m.det) + '</td>' +
          '<td>' + UI.esc(m.ndoc) + '</td><td class="mini">' + UI.esc(m.modulo || '—') + '</td><td class="mini">' + UI.esc(m.od) + '</td><td class="num">' + m.lineas.length + '</td><td class="num">' + UI.s(m.valor) + '</td></tr>';
      }), { vacio: 'Sin movimientos' }) +
      '<p class="hint">Todos los movimientos de la base compartida (Inventarios, Compras, Producción y Comercial): Ingresos, Salidas y Transferencias con su tipo de movimiento. Los de Producción llevan la orden de fabricación como documento. Aparecen también en Movimientos (GI-07) y Kardex (GI-06).</p>';
  },
  kardex() {
    const f = PR08.f;
    const arts = [...new Set(BD.d.movs.flatMap(m => m.lineas.map(l => l.art)))].sort();
    if (arts.indexOf(f.art) < 0) f.art = arts[0] || '';
    const filas = f.art ? Stock.kardex(f.art, f.kalm) : [];
    return '<div class="card"><div class="filters">' +
      UI.campo('Artículo', '<select onchange="PR08.f.art=this.value;App.refrescar()" style="max-width:420px">' + UI.opts(arts.map(a => ({ v: a, t: a + ' · ' + M.nomArt(a) })), f.art) + '</select>') +
      ...
      UI.tabla(['Fecha', 'Movimiento', 'Tipo de movimiento', 'Detalle', 'Documento', 'Almacén', ['Entrada', 'num'], ['Salida', 'num'], ['Costo', 'num'], ['Saldo en almacén', 'num']], filas.map(k =>
        '<tr><td class="mini">' + k.fecha + '</td><td><button class="btn-link" onclick="PR08.verMov(\'' + k.id + '\')">' + k.id + '</button></td><td class="mini">' + UI.esc((BD.mov(k.id) || {}).tipoMov || '') + '</td><td>' + UI.esc(k.det) + '</td><td>' + UI.esc(k.ndoc) + '</td><td class="mini">' + k.alm + '</td>' +
  ...
  verMov(id) {
    ...
    UI.modal({
      ...
      cuerpo: '<div class="formgrid c3">' + UI.dato('Detalle', UI.esc(m.det), { estilo: 'grid-column:span 2' }) + UI.dato('Estado', m.est) + UI.dato('Tipo de movimiento', UI.esc((m.tipoMov || '') + (m.tipoMovNom ? ' · ' + m.tipoMovNom : ''))) +
        UI.dato('Fecha', m.fecha) + UI.dato('Documento de origen', UI.esc(m.ndoc) + (BD.d.gres.filter(g => g.mov === m.id).map(g => '<br><span class="mini">GRE ' + UI.esc(g.id) + ' · ' + UI.esc(g.motivo) + '</span>').join(''))) + UI.dato('Módulo', UI.esc(m.modulo || '—')) +
...
const PR09 = {
  render() {
    ...
    const ofs = BD.d.ofs.filter(o => o.estado !== 'Cancelado');
    const cerr = ofs.filter(o => o.estado === 'Cerrado');
    const pt = ofs.filter(o => (M.art(o.art) || {}).grupo === 'PT');
