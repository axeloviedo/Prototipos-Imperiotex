...
const CM08 = {
  ...
  render() {
    const arts = Store.d.arts.filter(a => a.venta);
    ...
    html += '<div class="card"><div class="sec">Simulador</div><div class="formgrid c4">' +
      ...
      UI.tabla(['Nivel', 'Fila', ['Precio', 'num'], ''], niveles.map((n, i) => '<tr><td>' + (i + 1) + '. ' + n.t + '</td><td class="mini">' + (n.x ? n.x.id : '—') + '</td><td class="num">' + (n.x ? UI.m(n.x.precio, n.x.mon) : '') + '</td><td>' + (n.gana ? '<b class="ok-t">✓ se usa</b>' : n.x ? '<span class="mini">ignorado</span>' : '') + '</td></tr>')
        .concat(['<tr><td>5. Precio sugerido del artículo</td><td class="mini">GI-02</td><td class="num">' + (s.mon === 'PEN' ? UI.s(a.precio * Precios.factor(a.cod, s.um)) : '<span class="mini">solo PEN</span>') + '</td><td>' + (!usado && r ? '<b class="ok-t">✓ se usa</b>' : '') + '</td></tr>'])) +
  ...
  render() {
    ...
    return '<div class="screen-head"><h1>Artículos de venta</h1><span class="code">CL-43</span></div>' +
      ...
      UI.tabla(['Código', 'Artículo', 'Grupo', 'UM venta', ['Precio sugerido', 'num'], ['Precio mínimo', 'num'], ['Descuento', 'num'], 'IGV', 'Control de stock al vender', ['Disponible', 'num'], ['', '', '70px']], arts.map(a =>
        '<tr><td>' + a.cod + '</td><td>' + UI.esc(a.nom) + '</td><td class="mini">' + a.grupo + '</td><td>' + (a.uVenta || [a.u]).join(', ') + '</td>' +
        '<td class="num">' + UI.s(a.precio) + '</td><td class="num">' + (a.precioMin ? UI.s(a.precioMin) + '<br><span class="mini">' + (a.verifMin || Store.d.cfg.verificarPrecioMin ? 'se verifica' : 'no se verifica') + '</span>' : '—') + '</td>' +
        '<td class="num">' + a.dctoMin + '% – ' + a.dctoMax + '%</td><td class="mini">' + a.igv + '</td>' +
  ...
  editar(cod) {
    ...
    UI.modal({
      ...
      cuerpo: '<div class="formgrid">' + UI.dato('Artículo', UI.esc(a.nom), { full: true }) +
        ...
        UI.campo('Descuento mínimo (%)', '<input id="av-dmin" type="number" min="0" max="100" step="any" value="' + a.dctoMin + '">') +
        UI.campo('Descuento máximo (%)', '<input id="av-dmax" type="number" min="0" max="100" step="any" value="' + a.dctoMax + '">') +
