/* COMERCIAL V9 · CM-02 Ventas: listado, nueva venta (directa o desde cotización) y ficha de la venta */
const CM02 = {
  f: { q: '', sede: '', est: '', pago: '', tipo: '', mon: '', desde: '', hasta: '' },
  lista() {
    ...
    return Store.d.ventas.filter(v => (!f.sede || v.sede === f.sede) && (!f.est || v.estado === f.est) && (!f.pago || Ventas.estadoPago(v) === f.pago) &&
      (!f.tipo || Doc.tipoDoc(v) === f.tipo) && (!f.mon || v.mon === f.mon) && UI.enRango(v.fecha, f.desde, f.hasta) &&
  ...
  render() {
    ...
    return '<div class="screen-head"><h1>Ventas</h1><span class="code">CM-02</span><div class="spacer"></div>' +
      ...
      UI.kpis([
        ...
        { l: 'Pagos por validar', v: pv, s: 'se validan en Caja (CM-04)', color: 'var(--parcial)' },
      ...
      UI.campo('Pago', '<select onchange="CM02.f.pago=this.value;CM02.pintar()">' + UI.opts(['Pagado', 'Parcial', 'Pendiente de pago', 'Por devolver', 'Anulada'], f.pago, 'Todos') + '</select>') +
      UI.campo('Tipo', '<select onchange="CM02.f.tipo=this.value;CM02.pintar()">' + UI.opts(['Productos', 'Servicios', 'Mixta'], f.tipo, 'Todos') + '</select>') +
      ...
      '<p class="hint">Una venta es un solo documento para productos y servicios. Al registrarla se hace la <b>Salida</b> de almacén (GI-10, venta al por menor o al por mayor) desde el almacén de cada línea; no se edita después: se corrige con una devolución o, dentro del plazo, se anula.</p>';
  },
  pintar() {
    const filas = CM02.lista().map(v => {
      ...
      return '<tr class="clickable" onclick="App.go(\'cm02v\',{id:\'' + v.id + '\'})"><td><b>' + v.id + '</b><br><span class="mini">' + M.comp(v.comp).nom + ' ' + v.compNum + '</span></td>' +
        ...
        '<td class="mini">' + Doc.tipoDoc(v) + '</td><td class="mini">' + M.cond(v.cond).nom + (vto ? '<br>vence ' + vto : '') + '</td>' +
        '<td class="num">' + UI.m(v.total, v.mon) + '</td><td class="num"><span class="' + (Ventas.vencida(v) ? 'err-t' : '') + '">' + (deuda > 0.004 ? UI.m(deuda, v.mon) : '—') + '</span></td>' +
        '<td>' + UI.estado(Ventas.estadoPago(v)) + (Ventas.porValidar(v) ? '<br><span class="mini">' + Ventas.porValidar(v) + ' por validar</span>' : '') + '</td><td>' + UI.estado(v.estado) + '</td></tr>';
    });
    document.getElementById('cm02-body').innerHTML = UI.tabla(['Venta · comprobante', 'Fecha', 'Tienda', 'Cliente', 'Tipo', 'Condición', ['Total', 'num'], ['Saldo', 'num'], 'Pago', 'Estado'], filas, { vacio: 'No hay ventas con esos filtros' });
  },
  desdeCot() {
    const vig = Store.d.cots.filter(c => c.estado === 'Vigente');
    UI.modal({
      lg: true, titulo: 'Venta desde una cotización',
  ...
  excel(detalle) {
    const l = CM02.lista();
    if (!detalle) {
      UI.csv('ventas-generales', ['Venta', 'Comprobante', 'Número', 'Fecha', 'Tienda', 'Cliente', 'Documento', 'Tipo de cliente', 'Vendedor', 'Condición', 'Moneda', 'Op. gravada', 'Op. exonerada', 'IGV', 'Total', 'Devuelto', 'Pagado', 'Saldo', 'Estado de pago', 'Estado', 'Cotización'],
        l.map(v => [v.id, M.comp(v.comp).nom, v.compNum, v.fecha, v.sedeNom, v.cliente.nom, v.cliente.doc, v.cliente.tipo, DOCUI.vendedor(v.asesor), M.cond(v.cond).nom, v.mon, v.gravada, v.exonerada, v.igv, v.total, Ventas.devuelto(v), Ventas.pagado(v), Ventas.deuda(v), Ventas.estadoPago(v), v.estado, v.cot || '']));
      return;
    }
    const filas = [];
    l.forEach(v => v.lineas.forEach((x, i) => filas.push([v.id, v.compNum, v.fecha, v.cliente.nom, i + 1, x.art, x.nom, x.desc, x.um, x.alm, x.cant, x.precio, x.dcto, x.obsequio ? 'Sí' : '', x.subtotal, x.impuesto, x.total, v.mon, v.estado])));
    UI.csv('ventas-detalle', ['Venta', 'Comprobante', 'Fecha', 'Cliente', 'Línea', 'Código', 'Artículo', 'Descripción', 'UM', 'Almacén', 'Cantidad', 'Precio', 'Dcto. unit.', 'Obsequio', 'Base', 'IGV', 'Total', 'Moneda', 'Estado'], filas);
  }
};
...
const CM02F = {
  ...
  render(p) {
    ...
    let html = '<div class="screen-head"><h1>Nueva venta' + (bloq ? ' <span class="mini">desde ' + d.cot + '</span>' : '') + '</h1><span class="code">CM-02</span><div class="spacer"></div>' +
    ...
    html += ses ? UI.aviso('Caja <b>' + ses.id + '</b> abierta en ' + UI.esc(sede.nom) + ' (' + d.mon + '): los pagos entran a esta caja como <b>Por validar</b>.', 'ok')
      : UI.aviso('La caja de ' + UI.esc(sede.nom) + ' en ' + d.mon + ' está <b>cerrada</b>: solo se puede registrar una venta al crédito sin pago a cuenta. ' + (Store.puede('ver_caja') ? '<button class="btn-link" onclick="App.go(\'cm04\')">Ir a Caja</button>' : ''));
    if (bloq) html += UI.aviso('Conversión de <b>' + d.cot + '</b>: el cliente y las líneas vienen de la cotización y no se editan. Complete comprobante, fecha, entrega y pago; el stock se vuelve a revisar ahora.', 'info');

    html += '<div class="card"><div class="formgrid c4">' +
      UI.dato('Tienda', UI.esc(sede.nom) + '<br><span class="mini">vende desde ' + sede.alm + '</span>') +
      UI.campo('Fecha del documento', '<input type="date" value="' + UI.dIso(d.fecha) + '" max="' + UI.dIso(UI.hoy()) + '" onchange="CM02F.cab(\'fecha\',UI.dTxt(this.value))">', { req: true }) +
    ...
    if (d.lineas.length && !Doc.soloServicios(d)) {
      ...
      html += '<div class="card"><div class="sec">Entrega</div><div class="formgrid c4">' +
        ...
        '</div><p class="hint" style="margin-top:8px">La salida de almacén (GI-10) se registra al confirmar la venta. La guía de remisión, si el despacho la necesita, se emite en GI-15 con motivo Venta.</p></div>';
    }
    ...
    else html += UI.aviso('Lista para registrar. Al confirmar: salida de almacén de los productos' + (d.pagos.length ? ', pagos Por validar en la caja ' + ses.id : '') + (d.cot ? ' y ' + d.cot + ' pasa a Convertida' : '') + '.', 'ok');
  ...
  registrar() {
    ...
    if (r.w.length) UI.confirmar('Registrar con avisos de stock', '<p>Estos artículos se venden aunque el disponible no alcance (su control de stock es «Avisar y permitir»); el Stock Actual puede quedar negativo:</p><ul class="errlist">' + r.w.map(x => '<li>' + UI.esc(x) + '</li>').join('') + '</ul>', go, 'Registrar venta');
    else go();
  }
};
App.pantalla('cm02f', {
...
const CM02V = {
  ...
  render(p) {
    ...
    if (v.estado === 'Registrada' && Dev.candidatas(v).length && Store.puede('crear_devolucion_venta')) b.push('<button class="btn btn-secondary" onclick="App.go(\'cm03f\',{venta:\'' + v.id + '\',nuevo:Date.now()})">↩ Devolución</button>');
    ...
    let html = '<div class="screen-head"><h1>' + v.id + '</h1>' + UI.estado(v.estado) + ' ' + UI.estado(Ventas.estadoPago(v)) + ' <span class="mini">' + comp.nom + ' ' + v.compNum + '</span><span class="code">CM-02</span><div class="spacer"></div>' + b.join('') + '</div>';
    if (v.anulacion) html += UI.aviso('<b>Anulada</b> el ' + v.anulacion.f + ' por ' + UI.esc(v.anulacion.u) + ': ' + UI.esc(v.anulacion.motivo) + '. El stock volvió al almacén' + (Ventas.porDevolver(v) > 0.004 ? ' y hay <b>' + UI.m(Ventas.porDevolver(v), v.mon) + '</b> por devolver al cliente en Caja.' : '.'), 'err');
    else if (Ventas.porDevolver(v) > 0.004) html += UI.aviso('Hay <b>' + UI.m(Ventas.porDevolver(v), v.mon) + '</b> por devolver al cliente: se entrega en Caja (CM-04 › Devoluciones de dinero).');
    if (Ventas.vencida(v)) html += UI.aviso('Crédito vencido el ' + vto + ' con saldo ' + UI.m(deuda, v.mon) + '.', 'err');
    html += UI.kpis([
      { l: 'Total', v: UI.m(v.total, v.mon), s: Doc.tipoDoc(v) },
    ...
    const en = v.entrega;
    html += '<div class="card"><div class="formgrid c4">' +
      UI.dato('Comprobante', comp.nom + '<br><b>' + v.compNum + '</b>') + UI.dato('Fecha', v.fecha) + UI.dato('Tienda', UI.esc(v.sedeNom)) +
      ...
      UI.dato('Se puede anular hasta', v.estado === 'Registrada' ? v.plazoAnular : '') +
  ...
  t_pag(v) {
    ...
    return UI.tabla(['Pago', 'Registrado', 'Medio', 'Operación / voucher', ['Monto', 'num'], 'Caja', 'Estado', ['', '', '160px']], filas, { vacio: 'Sin pagos' + (Ventas.deuda(v) > 0.004 ? ': cobre con + Pago o desde Caja' : '') }) +
      (ree.length ? '<div class="sec">Devoluciones de dinero al cliente</div>' + UI.tabla(['N°', 'Fecha', 'Origen', ['Monto', 'num'], 'Estado', 'Movimiento de caja'], ree) : '') +
      '<p class="hint">Cada pago entra a la caja abierta de la tienda como <b>Por validar</b>; quien tiene el permiso valid_payments lo valida o lo rechaza (por ejemplo, si la transferencia no llegó). Solo lo validado cuenta en el arqueo.</p>';
  },
  t_dev(v, devs) {
    return UI.tabla(['Devolución', 'Fecha', 'Sustento', ['Unidades', 'num'], ['Total', 'num'], 'Dinero', 'Estado'], devs.map(x => {
  ...
  t_mov(v, devs, movs) {
    return UI.tabla(['Movimiento', 'Fecha', 'Tipo', 'Detalle', 'Documento', 'Almacén', 'Concepto contable', ['Valor', 'num']], movs.map(m =>
      ...
      '<td>' + UI.esc(m.det) + '</td><td>' + m.ndoc + '</td><td class="mini">' + m.alm + '</td><td class="mini">' + UI.esc(m.concepto) + '</td><td class="num">' + UI.s(m.valor) + '</td></tr>'), { vacio: 'Sin movimientos de stock (solo servicios)' }) +
  ...
  validar(id) { if (App.accion(() => Ventas.validarPago(CM02V.v(), id), 'Pago validado')) App.refrescar(); },
  rechazar(id) {
    UI.motivo('Rechazar pago ' + id, '<p>El pago queda Anulado y el saldo vuelve a estar pendiente.</p>', null,
      mot => { if (App.accion(() => Ventas.rechazarPago(CM02V.v(), id, mot), 'Pago rechazado')) { UI.cerrar(); App.refrescar(); } }, 'Rechazar');
  },
  anular() {
    const v = CM02V.v();
    UI.motivo('Anular ' + v.id, '<p>La venta no se borra: queda <b>Anulada</b>, el stock vuelve al almacén y lo cobrado queda por devolver en Caja. Solo hasta el ' + v.plazoAnular + ' y si no tiene devoluciones.</p>', M.MOTIVOS_ANULACION,
      mot => { if (App.accion(() => Ventas.anular(v, mot), v.id + ' anulada')) { UI.cerrar(); App.refrescar(); } }, 'Anular venta');
  }
};
App.pantalla('cm02v', {
