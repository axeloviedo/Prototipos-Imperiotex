...
const CM04 = {
  ...
  t_dev(s, pend) {
    ...
    return '<div class="sec">Pendientes de entregar al cliente (' + UI.esc(Store.sede(s.sede).nom) + ', ' + s.mon + ')</div>' +
      ...
      '<p class="hint">Se generan al anular una venta cobrada (lo pagado con nota de crédito vuelve al crédito del cliente) y con «Devolver en caja» desde una devolución (lo que sobra de una nota de crédito). Es un movimiento propio (no un egreso de caja chica), así el reporte los distingue.</p>';
  },
