/* COMPRAS · CO-01/02/03 Proveedores y Tipos de proveedor, CT-09 buscador de proveedor — HTML
   Conectado a la base compartida: BD.d.maestros.proveedores / tiposProveedor / condicionesPago
   2026-09-18: Grupo = Nacional / Internacional · Tipo = Telas, Avíos, Servicios, Generales */
Vistas.pantallas(String.raw`
  ...
  <section class="screen" id="scr-co01">
    <div class="screen-head">
      ...
      <button class="btn btn-secondary" onclick="renderGrupos();openModal('m-co03')">Tipos de proveedor</button>
    ...
    <div class="card">
      <div class="filters">
        ...
        <div class="field"><label>Grupo</label><select id="f-prv-g" onchange="renderProv()"><option value="">Todos</option><option>Nacional</option><option>Internacional</option></select></div>
        <div class="field"><label>Tipo de proveedor</label><select id="f-prv-t" onchange="renderProv()"><option value="">Todos</option></select></div>
    ...
    <div class="tbl-wrap">
      <table class="grid">
        <thead><tr><th>Código</th><th>Nombre</th><th>Grupo</th><th>Tipo de proveedor</th><th>Tipo y N° de documento</th><th>Condición de pago</th><th>Servicio que presta</th><th>Estado</th><th style="width:150px">Acciones</th></tr></thead>
        <tbody id="prv-body"></tbody>
      </table>
      <div class="pager"><span id="prv-count"></span></div>
    </div>
    <p class="hint">Maestro de proveedores de la base compartida (lo usan Inventarios, Compras y Producción). Un proveedor del grupo Internacional habilita compras en USD (importación). Los datos marcados «a confirmar» son supuestos del prototipo (RUC, condiciones) que el usuario debe validar.</p>
  </section>
    ...
    <div id="prv-pane-datos">
      <div class="card">
        ...
        <div class="formgrid" style="margin-top:12px">
          ...
          <div class="field"><label>Grupo <span style="color:var(--cancelada)">*</span></label>
            <select id="prv-grupo"><option value="">Seleccionar…</option><option>Nacional</option><option>Internacional</option></select></div>
          ...
          <div class="field"><label>Tipo de proveedor</label>
            <div style="display:flex;gap:8px"><select id="prv-tipo" style="flex:1"></select><button class="btn btn-secondary btn-sm" id="prv-b-tipos" onclick="renderGrupos();openModal('m-co03')">Gestionar</button></div></div>
...
<!-- CO-03 Tipos de proveedor (CRUD en modal) -->
<div class="overlay" id="m-co03">
  <div class="modal lg">
    <div class="modal-h"><b>Tipos de proveedor</b><span class="code" style="font-size:11px;color:var(--texto-sec)">CO-03</span><span class="x" onclick="closeModal('m-co03')">✕</span></div>
    <div class="modal-b">
      <div style="display:flex;gap:8px;margin-bottom:12px">
        <div class="field" style="flex:1"><label>Buscar tipo</label><input id="grp-q" placeholder="Buscar…" oninput="renderGrupos()"></div>
        <div class="field" style="flex:1"><label>Nombre del tipo de proveedor</label><input id="grp-nuevo" placeholder="Ej. Estampado"></div>
      ...
      <table class="grid subtable">
        <thead><tr><th style="width:80px">Código</th><th>Tipo</th><th>Descripción</th><th style="width:130px;text-align:right">Proveedores asignados</th><th style="width:150px">Acciones</th></tr></thead>
        <tbody id="grp-body"></tbody>
      </table>
      <p class="hint" style="margin-top:10px">Tipos de proveedor del maestro compartido (hoja «GRUPO DE PROVEEDORES» de la plantilla Excel): el proveedor guarda el código del tipo. Al crear uno, el código se propone con las primeras letras del nombre. No se elimina un tipo con proveedores asignados: reasígnelos antes. Renombrar no cambia el código. El <b>grupo</b> del proveedor es Nacional o Internacional.</p>
    </div>
  ...
  <div class="modal lg">
    ...
    <div class="modal-b">
      <div class="filters" style="margin-bottom:12px">
        ...
        <div class="field"><label>Grupo</label><select id="ct09-g" onchange="renderCT09()"><option value="">Todos</option><option>Nacional</option><option>Internacional</option></select></div>
        <div class="field"><label>Tipo de proveedor</label><select id="ct09-t" onchange="renderCT09()"><option value="">Todos</option></select></div>
      </div>
      <table class="grid subtable">
        <thead><tr><th>Código</th><th>Nombre</th><th>Tipo y N° de documento</th><th>Tipo de proveedor</th><th>Servicio</th><th>Grupo</th><th style="width:100px"></th></tr></thead>
